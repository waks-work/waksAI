...
function M.get_current_context()
    local file = bridge.get_current_path()
    local row, col = unpack(bridge.get_cursor_position())
    return {
        file = file,
        line = row,
        col = col,
...
function M.get_selected_code()
    local check_mode = bridge.get_current_mode()
    ...
    local buffer = 0
    local lines = bridge.fetch_buffer_content(sr - 1, er, buffer)
...
function M.get_surrounding_function()
    ...
    if has_ts then
        ...
        while node do
            ...
            then
                ...
                local lines = bridge.fetch_buffer_content(sr, er + 1, buffer)
                return table.concat(lines, "\n")
            end
            node = node:parent()
        end
    ...
    for i = row, scan_limit, -1 do
        local line = bridge.fetch_buffer_content(i - 1, i, buffer)[1] or ""
    ...
    local total_lines = bridge.get_bline_count(buffer)
    ...
    for _, line in ipairs(output) do
        ...
        if path and line_no and content then
            table.insert(snippets, {
                ...
                excerpt = bridge.trim_whitespace(content)
            })
        end
