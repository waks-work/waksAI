...
InlineAI.ns                 = bridge.get_namespace_id('inline_ai')
...
function InlineAI:get_current_selection()
    ...
    if mode:match("[vV\22]") then
        ...
        local lines     = bridge.fetch_buffer_content(start_pos[2], end_pos[2], self.current_buf)
...
function InlineAI:process_ai_response(original_text, ai_response)
    if not ai_response or ai_response == "" then
        bridge.notify("No AI response received", bridge.get_log_level("error"))
        return
    end

    local suggestions = self:parse_ai_response(original_text, ai_response)
    if not suggestions or vim.tbl_isempty(suggestions) then
        bridge.notify("No code suggestions found", bridge.get_log_level("warn"))
        return
    end
...
function InlineAI:parse_ai_response(original_text, ai_response)
    local suggestions         = {}
    local current_line        = bridge.line('.')

    suggestions[current_line] = {
        line_num       = current_line,
        original_lines = bridge.split_strings(original_text, '\n', true),
        ai_lines       = bridge.split_strings(ai_response, '\n', true),
        type           = 'modification',
...
function InlineAI:accept_current()
    local line = bridge.line('.')
    local suggestion = self:find_suggestion_at_line(line)

    if not suggestion then return end

    local file_name = bridge.get_current_file_name()
    ...
    bridge.notify("AI Change Applied & Recorded", bridge.get_log_level("info"))
end
...
function InlineAI:clear_suggestion(line_num)
    if self.suggestions[line_num] then
        bridge.clear_overlay(self.current_buf, self.ns)
...
function InlineAI:reject_current()
    ...
    if suggestion then
        ...
        api.record_activity(suggestion.session_id, "reject_suggestion",
            bridge.json_encode({ line = suggestion.line_num, file = bridge.get_current_file_name('%:t') }))

        self:clear_suggestion(suggestion.line_num)
        bridge.notify("AI suggestion rejected", vim.log.levels.INFO)
    end
end
...
function InlineAI:next_suggestion()
    ...
    if next_line then
        bridge.set_cursor_position(0, { next_line, 0 })
    end
end
...
function InlineAI:prev_suggestion()
    ...
    if prev_line then
        bridge.set_cursor_position(0, { prev_line, 0 })
    end
end
...
function InlineAI:goto_first_suggestion()
    ...
    if first_line then
        bridge.set_cursor_position(0, { first_line, 0 })
    end
end
