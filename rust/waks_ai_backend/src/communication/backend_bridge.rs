...
/// Central backend bridge — routes frontend requests to AI & Storage.
    ...
    match req {
        ...
        Request::RecordSession { session_id, status } => {
            ...
            match storage.record_session(&session).await {
                Ok(_) => Response::Success {
                    data: format!("🧾 Session '{}' recorded", session_id),
                },
                Err(e) => Response::Error {
                    message: format!("Failed to record session: {}", e),
                },
            }
        }
        ...
        Request::RecordResponse {
            ...
            match storage.record_response(&response).await {
                Ok(_) => Response::Success {
                    data: format!("💬 Response stored for session '{}'", session_id),
                },
                Err(e) => Response::Error {
                    message: e.to_string(),
                },
        ...
        // ======================================================
        // 🖥️ STRUCTURED: FRONTEND ACTIVITY
        // ======================================================
